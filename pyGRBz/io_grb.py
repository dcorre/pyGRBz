...
def _read_ascii(f, **kwargs):
    ...
    for line in f:
        ...
        if not readingdata:
            ...
            # Read header line
            if grb_format.lower() == 'lc': colnames.extend(['Name'])
            elif grb_format.lower() == 'sed': colnames.extend(['Name','time_since_burst'])
        ...
        if grb_format.lower() == 'sed': items.extend([time])
...
def load_telescope_transmissions(info_dict,wavelength,norm=False,norm_val=1.):
    """Load the transmittance of the selected band of the selected telescope with respect to wavelength
    ...
    if max(trans)>1: trans=np.array(trans, dtype=np.float64)*1e-2
    ...
    trans = resample(wvl,trans,wavelength,0.,1.)
    return trans
