...
def plot_lc_fit_check(observations, grb_info, lc_fit_params, model, plot,output_dir='/results/', filename_suffix=''):
    ...
    for obs_table in observations.group_by('Name').groups:
         ...
         colors = [cmap(i) for i in np.linspace(0, 1, len(lc_fit_params['band'][lc_fit_params['name']==obs_table['Name'][0]])+1)]

         plt.figure()
         #sort observations by eff. wavelength, telescope and band. keep just one time 
         for i,band_table in enumerate(obs_table.group_by(['eff_wvl','telescope','band']).groups.keys):
              #print (band_table)
              #select all observations with same band and telescope
              mask = (obs_table['band'] == band_table['band']) & (obs_table['telescope'] == band_table['telescope'])
              ...
              mask2 = (lc_fit_params['name'] == obs_table['Name'][0]) & (lc_fit_params['band'] == band_table['band']) & (lc_fit_params['telescope'] == band_table['telescope']) 
              #print (i,obs_table['detection'][mask])

              for t in range(len(time)):
                   plt.errorbar(time[t],y[t],xerr=xerr[t],yerr=yerr_[t],uplims=1-obs_table['detection'][mask][t],label=band_table['telescope']+' '+band_table['band'],color=colors[i])
              time_fit=np.linspace(time[0],time[-1],100)
              #print (lc_fit_params['F0'][mask2])
              if model == 'BPL':
                  plt.plot(time_fit,BPL_lc(time_fit,float(lc_fit_params['F0'][mask2]),float(lc_fit_params['norm'][mask2]),float(lc_fit_params['alpha1'][mask2]),float(lc_fit_params['alpha2'][mask2]),float(lc_fit_params['t1'][mask2]),float(lc_fit_params['s'][mask2])),label=band_table['telescope']+' '+band_table['band'],color=colors[i])
              elif model == 'SPL':
                 plt.plot(time_fit,SPL_lc(time_fit,float(lc_fit_params['F0'][mask2]),float(lc_fit_params['t0'][mask2]),float(lc_fit_params['norm'][mask2]),float(lc_fit_params['alpha'][mask2])),label=band_table['telescope']+' '+band_table['band'],color=colors[i])
         ...
         plt.grid(True)
         plt.savefig(output_dir+str(obs_table['Name'][0])+"/lc_fit_"+model+filename_suffix+".png")
         if plot: plt.show()
         plt.close('all')
...
def plot_sed(seds, grb_info, plot, model, output_dir='/results/', filename_suffix=''):
    ...
    for sed in seds.group_by('Name').groups:
         ...
         for i,band_table in enumerate(sed.group_by(['eff_wvl','telescope','band','band_width','mag','mag_err','detection']).groups.keys):
              if band_table['detection']==0: yerr=0.5
              else: yerr=band_table['mag_err']
              plt.errorbar(band_table['eff_wvl'],band_table['mag'],xerr=band_table['band_width']/2, yerr=yerr,lolims=1-band_table['detection'],label=band_table['telescope']+' '+band_table['band'],color=colors[i])
         ...
         plt.title('%s SED extracted at T-To=%.0f sec \n z=%.2f, Av_host=%.2f' % (sed['Name'][0],sed['time_since_burst'][0],z,Av_host))
         plt.grid(True)
         plt.legend(numpoints=1)
         plt.savefig(output_dir+str(sed['Name'][0])+"/extracted_sed_"+model+filename_suffix+".png")
         if plot: plt.show()
         plt.close('all')
...
def plot_mcmc_evolution(samples_corr, samples_del, nburn, ndim, ext_law, Av_sim, z_sim, name, plot, plot_deleted, output_dir='/test/', filename_suffix='', priors=dict(z=[0,11], Av=[0,2], beta=[0,2], norm=[0,10])):
   ...
   fig.gca().annotate("zsim: %.2f    Av_sim: %.2f " % (z_sim,Av_sim), xy=(0.5, 1.0), xycoords="figure fraction", xytext=(0, -5), textcoords="offset points", ha="center", va="top")
   fig.savefig(output_dir+str(name)+"/evolution_steps_"+ext_law+filename_suffix+".png")
   if plot: plt.show()
   plt.close(fig)
   plt.close('all')
...
def plot_triangle(samples, ndim, z_sim, ext_law, Av_sim, beta_sim, name, plot, plot_deleted, filename_suffix='', output_dir='/test/', priors=dict(z=[0,11],Av=[0,2],beta=[0,2],norm=[0,10])):
   ...
   if ndim ==3:
       fig = corner.corner(samples, labels=["z", "beta", "norm"],
                                            ...
                                            bins=50,
                                            range=[(priors['z'][0],priors['z'][1]),(priors['Av'][0],priors['Av'][1]),1.],
       ...
       fig.gca().annotate("zsim: %.2f    Av_sim: %.2f " % (z_sim,Av_sim), xy=(0.5, 1.0), xycoords="figure fraction", xytext=(0, -5), textcoords="offset points", ha="center", va="top")
       fig.savefig(output_dir+str(name)+"/triangle_plot_"+ext_law+filename_suffix+".png")
       #plt.title('zsim: %.2f    Av_sim: %.2f ' % (z_sim,Av_sim))

   elif ndim==4:
       fig = corner.corner(samples, labels=["z", "Av", "beta","norm"],
                                                 truths=[z_sim, Av_sim, -1,-1],
                                                 bins=50,
                                                 range=[(priors['z'][0],priors['z'][1]),(priors['Av'][0],priors['Av'][1]),(priors['beta'][0],priors['beta'][1]),1.],
                                                 levels=[0.68,0.95,0.99],
                                                 plot_contours=True,
                                                 fill_contours=True,
                                                 plot_datapoints=False,
...
def plot_mcmc_fit(results, ndim, results_minL, sed, wavelength, samples, plot_all, plot, ext_law, Host_dust, Host_gas, MW_dust, MW_gas, DLA, igm_att, output_dir='/test/', filename_suffix=''):
   ...
   print ('median values PDF: {:.3f} {:.3f} {:.3f} {:.3f}'.format(z_fit,Av_fit,beta_fit,norm_fit))

   print ('best fit: {:.3f} {:.3f} {:.3f} {:.3f}'.format(z_minL,Av_minL,beta_minL,norm_minL))
   
   flux_median=Flux_template(wavelength,F0,wvl0,norm_fit,beta_fit,z_fit,Av_fit,ext_law,Host_dust,Host_gas,MW_dust,MW_gas,DLA,igm_att)
   flux_minL=Flux_template(wavelength,F0,wvl0,norm_minL,beta_minL,z_minL,Av_minL,ext_law,Host_dust,Host_gas,MW_dust,MW_gas,DLA,igm_att)
   plt.figure()
   ...
   if plot_all:
       if ndim == 3:
           for z,beta,norm in samples:#[np.random.randint(len(samples), size=100)]:
               plt.plot(wavelength, Flux_template(wavelength,F0,wvl0,norm,beta,z,0,ext_law), color="k", alpha=0.01)
       elif ndim==4:
           for z,beta,norm,Av in samples:#[np.random.randint(len(samples), size=100)]:
               plt.plot(wavelength, Flux_template(wavelength,F0,wvl0,norm,beta,z,Av,ext_law), color="k", alpha=0.01)

   # Set color for plots
   cmap = plt.get_cmap('rainbow')
   colors = [cmap(i) for i in np.linspace(0, 1, len(sed)+1)]

   #plt.plot(wavelength, , label='True')
   for i in range(len(sed['band'])):
       #mask = seds['band'] == band
       if sed['detection'][i]==0: yerr=0.3
       else: yerr =sed['flux_err'][i]
       plt.errorbar(sed['eff_wvl'][i],sed['flux'][i],xerr=sed['band_width'][i]/2,yerr=yerr,uplims=1-sed['detection'][i],color=colors[i],lw=1.5)
       plt.annotate(sed['band'][i], xy=(sed['eff_wvl'][i],max(sed['flux'])*1.3),color=colors[i], fontsize=16, horizontalalignment='right', verticalalignment='bottom')
   ...
   #plt.ylim(1,400)
   plt.title('SED extracted at T-To=%.0f sec\n z_sim=%.2f, Av_sim=%.2f' % (float(sed['time_since_burst'][0]),float(z_sim),float(Av_sim)))
   plt.yscale('log')
   plt.grid(True)
   name_tel=sed['telescope'][0]
   if name_tel not in ['ratir','grond','gft']:name_tel='mix'
   plt.legend(loc='lower right',numpoints=1,title='%s\n (%s data)'%(sed['Name'][0],name_tel),fontsize=16)
   #set the legend title size
   plt.setp(plt.gca().get_legend().get_title(), fontsize='16') 
   plt.savefig(output_dir+str(sed['Name'][0])+'/bestfit_'+ext_law+filename_suffix+'.png')
   if plot: plt.show()
   plt.close('all')
...
def plot_zphot(input_file,output_suffix,sigma,input_dir='/results/',output_dir='/plots/',plot=True):
   ...
   #path='results/'
   #filename='best_fits_mcmc_68'
   filename = "%s%s" % (input_dir,input_file)
   #output_suffix='_test'
   #models= table.Table.read('test.dat')
   ...
   #models = np.load(fname_best_fits+'.npy')
   """
   print (results,type(results))
   results = np.array(results)
   results=models
   sort_results=np.array(sorted(results,key=lambda x: x[0]))
   zsim=sort_results[:,0]
   Avsim=sort_results[:,1]
   zphot=sort_results[:,2]
   z_sup=zphot+sort_results[:,3]
   z_inf=zphot-sort_results[:,4]
   """
   #print (list(zip(zsim,zphot,z_inf,z_sup)))
   ...
   # Set Av to -0.1 when not available from literature. It allows to keep a readable colorbar
   for k in range(len(Avsim)):
       if Avsim[k]==-99: Avsim[k]=-0.1

   plt.figure()
   #plt.figure(1)
   # If the number is higher than 16 this has to be extended.
   markers = ['x','s','*','v','o','^','D','<', '>', '8', 'p', 'h', 'H', 'd', 'P', 'X']
   ...
   #print (zsim_bin) 
   #plt.fill_between(zsim_bin, bin_means-bin_std, bin_means+bin_std)
   #print (Avsim)
   x=np.arange(0,12,1)
   ...
   #plt.legend(loc='upper left')
   #plt.title(r'Photometric redshift accuracy at 1$\sigma$')
...
def plot_zphot_comp():
   ...
   for i,filename in enumerate([filename_2,filename_1]):#,filename_2,filename_3]:
       f1=open(filename,'r')
       #print (best_fits)
       ...
       x=np.arange(0,12,1)
       #plt.scatter(zsim,zphot, label='best fit data',s=1, facecolor='0.7', lw = 0.8)
       plt.fill_between(list(zsim), list(z_inf), list(z_sup),color=color_file[i],alpha=0.3,label=label[i])
         

   plt.plot(x,x,ls='--',color='red',lw=0.5)
   plt.legend(loc='upper left')
   #plt.title(r'Photometric redshift accuracy at 1$\sigma$'+'\n'+r'$z_{sup}$ - $z_{inf}$ at 1$\sigma$')
   plt.xlabel(r'$z_{sim}$')
