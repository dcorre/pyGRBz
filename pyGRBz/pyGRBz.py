...
class GRB_photoZ():
   ...
   def load_data(self,data_dir='/data/sed/',data_name='GRB130606A'):
       ...
       if not isinstance(data_name,list): self.filenames=[data_dir+data_name+'.txt']
       else:
            self.filenames=[]
            for i , grbname in enumerate(data_name):
                ...
                if not os.path.exists(self.output_dir+grbname): os.makedirs(self.output_dir+grbname)
       ...
       #self.wavelength=np.arange(min(self.data['wvl_eff'])-2000,max(self.data['wvl_eff'])+2000,self.wvl_step).astype(np.float) 
       self.wavelength=np.arange(self.wvl_min,self.wvl_max,self.wvl_step).astype(np.float) 
   ...
   def extract_sed(self,model='SPL',method='ReddestBand',time_SED=1,output_dir='/results/',filename_suffix=''):
       ...
       print('\nSEDS:\n {}\n'.format(self.seds['Name','time_since_burst','band','mag','mag_err']))
       #self.seds=self.data
