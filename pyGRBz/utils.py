...
def convAB(wavelength,filter_trans):
    ...
    convAB=-2.5*np.log10(np.sum(flux_vega_resampled*filter_trans/wavelength*np.gradient(wavelength),axis=0) / np.sum(3631*filter_trans/wavelength*np.gradient(wavelength),axis=0))
    return convAB
